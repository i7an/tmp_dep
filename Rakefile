...
Rake::Task["release"].enhance do
  ...
  system "open https://github.com/i7an/tmp_dep/releases"
end
...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("tmp_dep.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in tmp_dep.gemspec include the following files
...
namespace :bump do
  ...
  task :ruby do
    replace_in_file "tmp_dep.gemspec", /ruby_version = .*">= (.*)"/ => RubyVersions.lowest
